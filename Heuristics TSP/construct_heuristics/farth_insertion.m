function [tour optimaldistance] = farthestinsertion(data)
...
for l = 1:size(data,1)-2
    Maximum_Value = 0;
    for b = 1:NonNaNVector(tour)-1
        for i = 1:size(Distance_data,2)
            if Distance_data(tour(1,b),i) ~= 0 && ...
                    ArrayChecker(tour,i) == false && ...
                    Distance_data(tour(1,b),i) > Maximum_Value
               Maximum_Value = Distance_data(tour(1,b),i); 
               Next_Node = i;
            end
        end
    end
