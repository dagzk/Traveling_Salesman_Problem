function [stringall tour optimaldistance] = randomdescent2opt(data)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                   START Random Descent 2-opt Method
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%find time constraint value from steepest 2-opt results and use as a
%benchmark
timeconstraint = readtable('steepestdescent2opt_result.csv');
index = table2array((timeconstraint(:,1)));
ind=find(ismember(index,instancename));
time = table2array(timeconstraint(ind,2))

%iterate 10 times and time iterations using tic/toc 
for iteration = 1:10
    tic
    stop = -1;
    ...
    while stop < 0 
        %randomly select two edges in the tour 
        i = randi(n-1);
        a = tour(i); 
        i = i+1;
        b = tour(i);
        ...
        z = (distanceMatrix(a,c) - distanceMatrix(c,d)) + distanceMatrix(b,d) - distanceMatrix(a,b);

        % Apply exchange
        if z < 0 %if there is an improvement
            if i < j
                tour(i:j-1) = tour(j-1:-1:i);
            else
                tour(j:i-1) = tour(i-1:-1:j);
            end
        end
        toc;
